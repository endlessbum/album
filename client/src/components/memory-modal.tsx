...
export default function MemoryModal({ memory, isOpen, onClose, onEdit, 'data-testid': testId }: MemoryModalProps) {
  ...
  const getMusicMeta = (url: string): { title: string; artist?: string; coverUrl?: string } => {
    // Пробуем все возможные ключи localStorage
    const keys = ['music_meta_v1', 'music_meta_v1_own'];
    // Пробуем также userId, если есть
    try {
    ...
    // Если не нашли — пробуем из тегов
          ...
          return (
            <div className="mt-4 mb-2 flex items-center gap-3 justify-center px-4">
              ...
              {meta.coverUrl ? (
                ...
                <div className="h-12 w-12 rounded bg-muted/60 flex items-center justify-center text-muted-foreground">
              ...
              <div className="flex flex-col min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-base text-foreground truncate max-w-[160px]">{meta.title}</span>
                  {meta.artist ? (
                    <span className="text-sm text-muted-foreground break-words">{meta.artist}</span>
                  ) : null}
                </div>
                {/* Дата убрана, чтобы не дублировать */}
              </div>
              {/* Кнопка play/pause */}
              <button
                className={`inline-flex items-center justify-center h-8 w-8 rounded-full ${isPlaying ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'} transition-colors ml-2`}
                onClick={() => {
                  if (isCurrent) player.toggle();
                  else player.playTrack({ url, title: meta.title || title, artist: meta.artist, coverUrl: meta.coverUrl });
                }}
