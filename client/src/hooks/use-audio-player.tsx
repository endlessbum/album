...
type AudioPlayerApi = AudioPlayerState & {
  ...
  updateTrackMeta: (url: string, meta: { title?: string; artist?: string; coverUrl?: string }) => void;
  next: () => void;
  prev: () => void;
  toggle: () => void;
...
export function AudioPlayerProvider({ children }: { children: React.ReactNode }) {
  ...
  const [state, setState] = useState<AudioPlayerState>({ playing: false, duration: 0, currentTime: 0, volume: 1, muted: false, current: null, queue: [], index: -1, repeat: 'all', shuffle: false });
  ...
  useEffect(() => { currentUrlRef.current = currentUrlVal; }, [currentUrlVal]);
  useEffect(() => { repeatRef.current = repeatVal; }, [repeatVal]);
  ...
  useEffect(() => {
    ...
    const onPause = () => {
      ...
      try { savePos(currentUrlRef.current, el.currentTime); } catch {}
    };
    const onEnd = () => {
    ...
    try {
      ...
      if (raw) {
        ...
        if (t && t.url) {
          ...
          setState(s => ({ ...s, current: t, currentTime: lastPos, duration: lastDur, queue: restoredQueue || s.queue, index: restoredIndex }));
  ...
  useEffect(() => {
    try {
      ...
      if (!m) {
        try { localStorage.setItem(MODE_KEY, rep); } catch {}
      }
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    ms.setActionHandler('seekbackward', (details: any) => { seek((audioRef.current?.currentTime || 0) - (details?.seekOffset || 10)); });
    ms.setActionHandler('seekforward', (details: any) => { seek((audioRef.current?.currentTime || 0) + (details?.seekOffset || 10)); });
    ms.setActionHandler('seekto', (details: any) => { if (typeof details?.seekTime === 'number') seek(details.seekTime); });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Update metadata and playback state
  const metaTitle = state.current?.title || (state.current?.url ? state.current.url.split('/').pop() || 'Аудио' : 'Аудио');
  ...
  useEffect(() => {
    ...
    try {
      ...
      ms.metadata = new (window as any).MediaMetadata({ title: metaTitle, artist: metaArtist, artwork });
    } catch {}
  ...
  const ensureAudioGraph = useCallback(async () => {
    try {
      ...
      if (!srcNodeRef.current && audioCtxRef.current) {
        ...
        gainRef.current.gain.setValueAtTime(gainRef.current.gain.value || 1, audioCtxRef.current.currentTime);
      }
  ...
  const setCurrentAndSrc = useCallback((track: TrackMeta) => {
    const el = audioRef.current!;
    try { fadeTo(0, 120); } catch {}
    el.src = track.url;
    setState(s => ({ ...s, current: track, currentTime: 0, duration: 0 }));
    try { localStorage.setItem(TRACK_KEY, JSON.stringify(track)); } catch {}
  }, [fadeTo, setState]);

  const playTrack = useCallback((track: TrackMeta) => {
    const el = audioRef.current!;
    if (!track?.url) return;
  ...
  const playList = useCallback((tracks: TrackMeta[], startIndex: number) => {
  ...
  try { localStorage.setItem(QUEUE_KEY, JSON.stringify({ queue: list, index: idx })); } catch {}
    if (list.length) {
  ...
  const updateTrackMeta = useCallback((url: string, meta: { title?: string; artist?: string; coverUrl?: string }) => {
    setState(s => {
      const newQueue = s.queue.map(t => t.url === url ? { ...t, ...meta } : t);
      let newCurrent = s.current;
      if (s.current?.url === url) {
        newCurrent = { ...s.current, ...meta };
        // Синхронизируем localStorage для восстановления после перезагрузки
        try { localStorage.setItem(TRACK_KEY, JSON.stringify(newCurrent)); } catch {}
      }
  // Обновим и сохранённую очередь
  try { localStorage.setItem(QUEUE_KEY, JSON.stringify({ queue: newQueue, index: s.index })); } catch {}
  ...
  const next = useCallback(() => {
    setState(s => {
      ...
      if (ni >= s.queue.length) {
        if (s.repeat === 'all') ni = 0; else return { ...s };
      }
      ...
      el.src = track.url;
      try { localStorage.setItem(TRACK_KEY, JSON.stringify(track)); } catch {}
  try { localStorage.setItem(QUEUE_KEY, JSON.stringify({ queue: s.queue, index: ni })); } catch {}
      const p = el.play();
      if (p && typeof p.then === 'function') p.catch(() => {});
      ensureAudioGraph().then(() => {
        try {
  ...
  const prev = useCallback(() => {
    setState(s => {
      ...
      if (pi < 0) {
        if (s.repeat === 'all') pi = s.queue.length - 1; else return { ...s };
      }
      ...
      el.src = track.url;
      try { localStorage.setItem(TRACK_KEY, JSON.stringify(track)); } catch {}
  try { localStorage.setItem(QUEUE_KEY, JSON.stringify({ queue: s.queue, index: pi })); } catch {}
      const p = el.play();
      if (p && typeof p.then === 'function') p.catch(() => {});
      ensureAudioGraph().then(() => {
        try {
  ...
  const seek = useCallback((time: number) => {
    const el = audioRef.current!;
    try { el.currentTime = Math.max(0, Math.min(time, el.duration || time)); } catch {}
  }, []);
  ...
  const close = useCallback(() => {
    ...
    setState(s => ({ ...s, playing: false, current: null, currentTime: 0, duration: 0, queue: [], index: -1 }));
    try {
  ...
  const toggleShuffle = useCallback(() => {
    setState(s => {
      ...
      try { localStorage.setItem(SHUF_KEY, nextVal ? '1' : '0'); } catch {}
  ...
  const toggleMute = useCallback(() => {
    ...
    try { localStorage.setItem(MUTE_KEY, el.muted ? '1' : '0'); } catch {}
  }, []);
  ...
  const api: AudioPlayerApi = useMemo(() => ({
    ...
    toggleShuffle,
    toggleMute,
  }), [state, playTrack, playList, updateTrackMeta, next, prev, toggle, seek, setVolume, stop, close, setRepeat, toggleShuffle, toggleMute]);
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      else if (e.code === 'ArrowUp') { setVolume(Math.min(1, (audioRef.current?.volume || 0) + 0.05)); }
      else if (e.code === 'ArrowDown') { setVolume(Math.max(0, (audioRef.current?.volume || 0) - 0.05)); }
