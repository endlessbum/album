...
export default function ChatPage() {
  ...
  useEffect(() => {
    function updateFromStorage(_e?: StorageEvent | CustomEvent) {
      try {
        setChatBgKey((localStorage.getItem('ui:chatBackground') as any) || 'none');
      } catch {}
    }
  ...
  const sendEphemeralMedia = async (file: File, kind: 'photo' | 'video') => {
    try {
      ...
      const mediaUrlAbs =
        j.url &&
        (j.url.startsWith('http') ? j.url : new URL(j.url, window.location.origin).toString());
      sendMessageMutation.mutate({
        content: '',
        type: msgType,
        isEphemeral: true,
        mediaUrl: mediaUrlAbs,
      });
    } catch (_e) {
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden w-full" data-testid="chat-page">
      <main className="flex-1 flex min-w-0">
        {/* Центрированный контейнер */}
        <div className="max-w-4xl w-full mx-auto px-3 sm:px-6 lg:px-0 h-full box-border pb-[88px] flex flex-col min-w-0">
          {/* Заголовок чата */}
          <div
            className="glass-strong p-3 sm:p-4 mt-2 sm:mt-3 mb-3 rounded-xl hover-lift"
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  {partnerData?.partner?.isOnline && (
                    <div
                      className={`absolute -bottom-1 -right-1 w-3 h-3 bg-online rounded-full border-2 border-background ${partnerTyping ? 'animate-pulse' : 'pulse-online'}`}
                ...
                <div>
                  ...
                  <p
                    className={`text-sm ${partnerTyping ? 'text-primary animate-pulse' : 'text-muted-foreground'}`}
          ...
          <div
            className="glass-strong mb-0 rounded-xl p-3 sm:p-4 hover-lift"
            data-testid="message-input-container"
            style={{ position: 'sticky', bottom: 0, zIndex: 10 }}
          >
            <form
              onSubmit={handleSendMessage}
              className="flex flex-wrap gap-2 sm:gap-3 items-center md:items-end w-full"
            >
