...
export default function MusicPage() {
  ...
  // Ключи localStorage для синхронизации
  ...
  // Универсальная синхронизация музыки между вкладками/устройствами
  useEffect(() => {
    function updateFromStorage(_e?: StorageEvent | CustomEvent) {
      try {
        setFavorites(getFavorites());
        setAlbums(getAlbums());
        setAudios((audios) => [...audios]); // триггерим обновление (метаданные/порядок)
      } catch {}
    }
  ...
  // (объявления ключей перенесены выше)
  ...
  const upsertMeta = (url: string, meta: { title: string; artist?: string; coverUrl?: string }) => {
    try {
      ...
      // Синхронизация с глобальным плеером, если он есть
      if (typeof window !== 'undefined') {
        const playerCtx = (window as any).audioPlayerCtx;
  ...
  const uploadWithMeta = (file: File, title: string, artist?: string) => {
    ...
    xhr.onload = () => {
      ...
      try {
        ...
        toast({
          title: 'Ошибка',
          description: err.message || 'Не удалось загрузить аудио',
          variant: 'destructive',
        });
      } finally {
        setMetaOpen(false);
  ...
  const saveEdit = () => {
    ...
    const apply = (coverUrl?: string) => {
      ...
      try {
        (player as any).updateTrackMeta?.(editItem.url, {
          title: newTitle,
          artist: newArtist,
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (canceled) return;
        // Используем только 'music_meta_v1_own' для метаданных
        let meta: Record<string, { title?: string; artist?: string; coverUrl?: string }> = {};
        try {
          meta = JSON.parse(localStorage.getItem('music_meta_v1_own') || '{}');
        } catch {}
        ...
        setAudios(items);
        // Синхронизируем названия треков с глобальным плеером при загрузке списка
        if (typeof window !== 'undefined') {
          const playerCtx = (window as any).audioPlayerCtx;
          if (playerCtx && typeof playerCtx.updateTrackMeta === 'function') {
            items.forEach((item) => {
  ...
  }, [applySavedOrder, viewingPartnerMusic, partnerData?.partner?.id]); // Reload when switching users
  ...
  return (
    <div className="flex min-h-screen w-full overflow-x-hidden min-w-0">
          ...
          <div className="mb-4 sm:mb-6 relative flex flex-col sm:grid sm:grid-cols-[auto,1fr,auto] items-center gap-4 min-w-0 overflow-x-auto">
            ...
            <div className="w-full sm:w-auto sm:absolute sm:left-1/2 sm:-translate-x-1/2 z-10 order-3 sm:order-none mt-2 sm:mt-0">
              <div className="relative flex items-center">
                <SearchIcon className="absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground pointer-events-none z-20" style={{maxHeight: '2rem'}} />
                <Input
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  placeholder="Поиск..."
                  className="pl-8 w-full sm:w-[27rem] max-w-full sm:max-w-[90vw] glass focus-ring text-center placeholder:text-center"
          ...
          {filtered.length === 0 ? (
            <div className="glass rounded-xl p-6 sm:p-8 text-center w-full max-w-[900px]">
              <p className="text-muted-foreground text-lg">
                Аудио не найдено. Добавьте файл или измените запрос.
              </p>
            </div>
          ) : (
            <div
              className="glass rounded-xl w-full max-w-[700px] divide-y divide-border/50 overflow-hidden"
                              ...
                              {currentView === 'favorites' ? (
                                ...
                                <>
                                  <DropdownMenuItem onClick={() => toggleFavorite(a.url)}>
                                    <Heart
                                      className={`h-4 w-4 ${favorites.includes(a.url) ? 'fill-current text-red-500' : ''}`}
                                    />
